...
router.get('/lightRailEta', async (req, res) => {
    ...
    if (!routeNo || !direction || (direction !== 1 && direction !== 2)) {
        res.status(400).send(
            'Bad Request - Must include both parameters: <br />routeNo (see https://opendata.mtr.com.hk/doc/LR_Next_Train_DataDictionary_v1.0.pdf)<br />direction (1 or 2)'
        )
    } else {
...
router.get('/get-news', async (req, res) => {
    try {
        const news = await fs.promises.readFile(
            './public/rthk-news.json',
            'utf-8'
        )
