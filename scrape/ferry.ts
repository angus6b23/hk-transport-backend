...
const fetchFerry = async () => {
    ...
    try {
        const ferryResponse = await axios(
            'https://static.data.gov.hk/td/routes-fares-geojson/JSON_FERRY.json'
        ) //Get all ferry information from data.gov.hk
        const ferryObj = ferryResponse.data.features
        // const busesObj = busesResponse.features;
        let ferries: FerryRoute[] = ferryObj.reduce(function (
            ferries: FerryRoute[],
            item: any
        ) {
            //reduce(function (accumulator, currentValue) { ... }, initialValue)
            const newStop = createStop<Stop>(item)
            const checkIndex = ferries.findIndex(
                (ferry) =>
                    ferry.routeId == item.properties.routeId &&
                    ferry.direction == item.properties.routeSeq
