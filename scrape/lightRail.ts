...
const fetchLightRail = async () => {
    ...
    try {
        ...
        let lr: MTRRoute[] = lrData.reduce(function (lr: MTRRoute[], item: any){
            const newStop: MTRStop = createMTRStop(item);
            const checkIndex = lr.findIndex(route => route.direction == item['Direction'] && route.routeId == item['Line Code']);
            if (checkIndex !== -1){
        ...
        for (let route of lr){
            let {code: originCode, nameEN: originEN, nameTC:originTC } = route.stops[0];
            let {code: destCode, nameEN: destEN, nameTC:destTC} = route.stops[route.stops.length - 1];
