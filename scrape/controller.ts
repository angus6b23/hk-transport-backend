...
const fetchAll = async (config: Config, types: TransportType[] = []) => {
    ...
    if (types.length > 0) {
        //Arguments will be passed from rebuildCache.ts
        let promiseArray = []
    ...
    console.info(
        chalk.green(
            `[scrape] Finished fetching and creating all transport type`
        )
    )
}
...
const writeChunked = async (array: any[], prefix: string) => {
    for (let i = 0; i < array.length; i++) {
        await fs.promises.writeFile(
            `./public/chunked/${prefix}-chunk${i}.json`,
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (data && data.length > 1) {
                ...
                console.info(
                    chalk.blue(`[scrape] Finish creating json for ${type}`)
                )
                resolve(undefined)
            }
            reject
