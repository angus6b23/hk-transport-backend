...
const fetchBuses = async () => {
    ...
    try {
        ...
        let buses: BusRoute[] = busesObj.reduce(function (
            buses: BusRoute[],
            item: any
        ) {
            //reduce(function (accumulator, currentValue) { ... }, initialValue)
            if (
                item.properties.companyCode == 'NLB' ||
            ...
            const checkIndex = buses.findIndex(
                (bus) =>
                    bus.routeId == item.properties.routeId &&
                    bus.direction == item.properties.routeSeq
...
const implementKMB = async (buses: BusRoute[]): Promise<BusRoute[]> => {
    ...
    try {
        ...
        const kmbResponse = await axios(
            'https://data.etabus.gov.hk/v1/transport/kmb/route/'
        )
        const { data: kmbJson } = kmbResponse
        const kmbFiltered = kmbJson.data.filter(
            (kmb: any) => kmb.serviceType != '1'
        )
        ...
        for (const specialRoute of kmbFiltered) {
            const checkIndex = buses.findIndex(
                (bus) =>
                    bus.serviceMode != 'R' &&
                    bus.routeNo === specialRoute.route &&
                    bus.destEN
                        .replace(checkParenthesis, '')
                        .replaceAll(' ', '') ==
                        specialRoute.dest_en
                            .replace(checkParenthesis, '')
                            .replaceAll(' ', '')
            )
            if (checkIndex != -1) {
        ...
        for (let kmbBus of kmbBuses) {
            ...
            if (timetable) {
                for (const slot of timetable) {
                    const checkIndex = newTimeTable.findIndex(
                        (nslot) =>
                            nslot.title === slot.DayType.replaceAll(' ', '')
                    )
                    if (
                        checkIndex === -1 &&
                        (
        ...
        console.error(
            chalk.red(`[bus] Error while implementing KMB API: ${err}`)
        )
        return buses
    }
}
...
const implementCTB = async (buses: BusRoute[]): Promise<BusRoute[]> => {
    ...
    const ctbBuses = buses.filter(
        (bus) =>
            bus.company.length == 1 &&
            (bus.company.includes('CTB') || bus.company.includes('NWFB'))
    )
    ...
    try {
        ...
        const ctbIdReq = ctbBuses.map(async (ctbBus) => {
            const company = ctbBus.company.includes('CTB')
                ? 'CTB'
                : ctbBus.company.includes('NWFB')
                  ? 'NWFB'
                  : null
            const direction = ctbBus.direction == 1 ? 'outbound' : 'inbound' //Direction 1 = outbound, 2 = inbound
            return axios(
                `https://rt.data.gov.hk/v1.1/transport/citybus-nwfb/route-stop/${company}/${ctbBus.routeNo}/${direction}`
            )
        })
        // Fetch all routes,
        ...
        for (let i = 0; i < ctbIdRes.length; i++) {
            ...
            for (let j = 0; j < targetRoute.length; j++) {
                const targetStop = stopRes.find(
                    (axios: any) =>
                        axios.config.url ==
                        `https://rt.data.gov.hk/v1.1/transport/citybus-nwfb/stop/${targetRoute[j].stop}`
                )
                const newStop: BusStop = {
                    ...
                    coord: [
                        targetStop?.data.data.long,
                        targetStop?.data.data.lat,
                    ],
                    etas: [],
                }
        ...
        const mixedBusIdReq = mixedBuses.map((bus) => {
            const company = bus.company.includes('CTB')
                ? 'CTB'
                : bus.company.includes('NWFB')
                  ? 'NWFB'
                  : null
            const direction = bus.direction == 1 ? 'outbound' : 'inbound' //Direction 1 = outbound, 2 = inbound
            return axios(
                `https://rt.data.gov.hk/v1.1/transport/citybus-nwfb/route-stop/${company}/${bus.routeNo}/${direction}`
            )
        })
        const mixedBusIdRes = await axios.all(mixedBusIdReq)
        for (let i = 0; i < mixedBusIdRes.length; i++) {
            let idRes = mixedBusIdRes[i].data.data
            let firstStopReq = await axios(
                `https://rt.data.gov.hk/v1.1/transport/citybus-nwfb/stop/${idRes[0].stop}`
            )
            let firstStopName = firstStopReq.data.data.name_en
                .slice(0, 5)
                .toLowerCase()
            if (
                mixedBuses[i].stops[0].nameEN
                    .toLowerCase()
                    .includes(firstStopName)
            ) {
                //Simply populate altId if the name of first stop matches
                // console.log(chalk.grey(`[bus] Successfully find bus ${idRes[0].route}`));
                mixedBuses[i].CTBDirection =
                    mixedBuses[i].direction == 1 ? 'O' : 'I'
                ...
                let filtered = mixedBuses.filter(
                    (bus) =>
                        bus.routeNo == idRes[0].route &&
                        bus.stops[0].nameEN
                            .toLowerCase()
                            .includes(firstStopName)
                )
                ...
                if (filtered.length > 2) {
                    // Prioritize filling buses with regular service mode if there are more than two routes fulfilling the criteria
                    findIndex = mixedBuses.findIndex(
                        (bus) =>
                            bus.serviceMode === 'R' &&
                            bus.routeNo == idRes[0].route &&
                            bus.stops[0].nameEN
                ...
                if (findIndex != -1) {
                    ...
                    mixedBuses[findIndex].CTBDirection =
                        mixedBuses[findIndex].direction == 1 ? 'I' : 'O'
                    for (let j = 0; j < idRes.length; j++) {
                    ...
                    console.warn(
                        chalk.yellow(
                            `[bus] Unable to find bus for route ${idRes[0].route}`
                        )
                    )
                }
            }
...
const implementNLB = async (buses: BusRoute[]): Promise<BusRoute[]> => {
    try {
        ...
        const originRegex = /^.*\>/
        const destRegex = /\>.*$/
        const { data: NLBRoutes } = await axios(
            'https://rt.data.gov.hk/v2/transport/nlb/route.php?action=list'
        )
        for (let route of NLBRoutes.routes) {
            let sameRouteNoExist = NLBBuses.some(
                (bus) => bus.routeNo == route.routeNo
            )
            let newRoute: BusRoute = {
                ...
                specialType: 0,
                serviceMode:
                    route.overnightRoute == 1
                        ? 'N'
                        : route.specialRoute == 1
                          ? 'T'
                          : 'R',
                originTC: route.routeName_c
                    .match(originRegex)[0]
                    .replace(' >', ''),
                originEN: route.routeName_e
                    .match(originRegex)[0]
                    .replace(' >', ''),
        ...
        const NLBStopsReq = NLBBuses.map((bus) =>
            axios(
                `https://rt.data.gov.hk/v2/transport/nlb/stop.php?action=list&routeId=${bus.routeId}`
            )
        )
        ...
        console.error(
            chalk.red('[bus] Error while implementing NLB Buses: ' + err)
        )
        return buses
    }
}
...
const implementMTR = async (buses: BusRoute[]): Promise<BusRoute[]> => {
    try {
        ...
        const { data: mtrBusResponse } = await axios(
        ...
        for (let item of mtrBusData) {
            let {
                ...
                STATION_NAME_CHI: nameTC,
                STATION_NAME_ENG: nameEN,
            } = item
            direction = direction == 'O' ? 1 : 2
            const index = mtrBuses.findIndex(
                (bus) => bus.routeNo == routeNo && bus.direction == direction
            )
            let newStop: BusStop = {
        ...
        const { data: mtrBusRouteResponse } = await axios(
            'https://opendata.mtr.com.hk/data/mtr_bus_routes.csv'
        )
        const mtrBusRouteData: any[] = papa.parse(
            mtrBusRouteResponse,
            PAPACONFIG
        ).data
        ...
        for (const item of mtrBusRouteData) {
            let {
                ROUTE_ID: routeNo,
                ROUTE_NAME_CHI: nameTC,
                ROUTE_NAME_ENG: nameEN,
            } = item
            mtrBusRoute.push({
                routeNo: routeNo,
        ...
        for (let bus of mtrBuses) {
            let targetBus = mtrBusRoute.find(
                (item) => item.routeNo == bus.routeNo
            )
            if (bus.direction == 1) {
        ...
        console.error(
            chalk.red(`[bus] Error while implementing MTR buses API: ${err}`)
        )
        return buses
    }
}
