...
export default async function fetchRthkNews () {
    try{
        const res: AxiosResponse = await axios('https://programme.rthk.hk/channel/radio/trafficnews/index.php');
        const {window} = new jsdom.JSDOM(res.data)
        let messages: Message[] = []
        const messageBlock: HTMLUListElement[] = window.document.querySelectorAll('ul.dec')
        messageBlock.forEach((block) => {
            const message = block.querySelector('li.inner')?.childNodes[0].nodeValue
