...
const fetchMinibus = async () => {
    ...
    try {
        const minibusesResponse = await axios(
        ...
        let minibuses: MinibusRoute[] = minibusesObj.reduce(function (
            minibuses: MinibusRoute[],
            item: any
        ) {
            //reduce(function (accumulator, currentValue) { ... }, initialValue)
            const newStop = createStop<Stop>(item)
            const checkIndex = minibuses.findIndex(
                (minibus) =>
                    minibus.routeId == item.properties.routeId &&
                    minibus.direction == item.properties.routeSeq
            ) //Check if route of current stop is stored
            if (checkIndex == -1) {
                //Create route if not found
                const newMinibusRoute = createRoute<MinibusRoute>(
                    item,
                    'minibus'
                )
        ...
        console.error(
            chalk.red(`[minibus] Error while scraping minibus: ${err}`)
        )
    }
}
...
const getRouteStops = async (minibuses: MinibusRoute[]) => {
    try {
        ...
        const stopReq = minibuses.map((route) =>
            axios.get(
                `https://data.etagmb.gov.hk/route-stop/${route.routeId}/${route.direction}`
            )
        )
        ...
        for (let i = 0; i < stopRes.length; i++) {
            ...
            if (newStopList.length == 0) {
                console.warn(
                    chalk.yellow(
                        `[minibus] Minibus No ${minibuses[i].routeNo} stops not found`
                    )
                )
            }
            minibuses[i].stops =
                newStopList.length > 0 ? newStopList : minibuses[i].stops
        }
    } catch (err) {
        console.error(
            chalk.red(`[minibus] Error while getting minibuses stops: ${err}`)
        )
    }
    return minibuses
}
...
const getStopCoords = async (minibuses: MinibusRoute[]) => {
    try {
		 ...
		 */
        const allReq = Array.from(stopIdList).map((id) =>
            axios.get(`https://data.etagmb.gov.hk/stop/${id}`)
        )
        ...
        console.log(
            chalk.red(
                `[minibus] Error while getting minibus stops coord: ${err}`
            )
        )
        return minibuses
    }
