...
const fetchTram = async () => {
    ...
    try {
        ...
        let trams: TramRoute[] = tramObj.reduce(function (trams: TramRoute[], item: any) {
            //reduce(function (accumulator, currentValue) { ... }, initialValue)
            const newStop = createStop<Stop>(item);
            const checkIndex = trams.findIndex(tram => tram.routeId == item.properties.routeId && tram.direction == item.properties.routeSeq); //Check if route of current stop is stored
